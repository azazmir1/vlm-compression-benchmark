"""
Quantize VLMs with GPTQ INT8 and upload to HuggingFace Hub.

Usage:
  python scripts/quantize_gptq_int8_upload.py --model_id google/gemma-3-4b-it
  python scripts/quantize_gptq_int8_upload.py --all
"""
...
def make_calib_tokenized(tokenizer, n_calib: int):
    """Fallback: tokenized calibration data for models that don't support conversation format."""
    prompts = [
        "The quick brown fox jumps over the lazy dog.",
        "A visual question answering model processes images and text.",
        "Machine learning models can be compressed using quantization.",
    ...
    return [{"input_ids": tokenizer(t, return_tensors="pt")["input_ids"].squeeze().tolist()} for t in texts]


def make_repo_name(model_id: str) -> str:
    short = model_id.split("/")[-1]
    return f"{short}-GPTQ-Int8"
...
def quantize_and_upload(model_id: str, n_calib: int = 256):
    ...
    # Load model
    logger.info(f"Loading {model_id} for GPTQ INT8 quantization...")
    try:
    ...
    # Try conversation format first, fallback to tokenized
    logger.info(f"Preparing calibration data ({n_calib} samples)...")
    calib_data = make_calib_conversations(n_calib)

    logger.info("Running GPTQ INT8 quantization...")
    try:
        ...
        try:
            ...
            calib_data = make_calib_tokenized(tokenizer, n_calib)
            model.quantize(calib_data, batch_size=1, tokenizer=tokenizer)
    ...
    try:
        ...
        api.upload_folder(
            ...
            repo_id=full_repo,
            commit_message=f"GPTQ INT8 quantization of {model_id}",
        )
