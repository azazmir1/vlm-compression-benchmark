#!/usr/bin/env bash
# Run all pruning experiments (20% + 40% sparsity) across 7 GPUs in waves.
...
run_pruning_job() {
  ...
  local log_file="${LOG_DIR}/${safe_name}__${sp_tag}.log"

  echo "$(date '+%H:%M:%S') | GPU $gpu | START | $model_id (sp=$sparsity)"
  CUDA_VISIBLE_DEVICES=$gpu python compression/pruning/run_pruning.py \
    --model_id "$model_id" \
    --sparsity "$sparsity" \
    --vqav2_n 1000 \
    --skip_textvqa \
  ...
  local status=$?
  if [ $status -eq 0 ]; then
    echo "$(date '+%H:%M:%S') | GPU $gpu | DONE  | $model_id (sp=$sparsity)"
  else
    echo "$(date '+%H:%M:%S') | GPU $gpu | FAIL  | $model_id (sp=$sparsity) (exit=$status)"
  fi
  return $status
}
...
echo "================================================================"
echo " Pruning Experiments: $total jobs across 7 GPUs"
...
while [ $idx -lt $total ]; do
  ...
  for gpu in $(seq 0 $((batch_size - 1))); do
    ...
    model_id="${job%%|*}"
    sparsity="${job##*|}"
    run_pruning_job $gpu "$model_id" "$sparsity" &
    pids+=($!)
  done
...
echo "================================================================"
echo "Results in: results/pruning/"
ls results/pruning/*.json 2>/dev/null | wc -l
