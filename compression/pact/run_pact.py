...
class PACTTokenCompressor:
    ...
    def _pact_hook(self, module, args):
        ...
        Instead of physically removing tokens (which breaks rotary position
        embeddings and attention masks), we zero out pruned/merged tokens
        in-place so the sequence length stays constant.
        """
        if not args:
            return args
        ...
        # Work on a clone to avoid in-place issues with autograd
        hidden_states = hidden_states.clone()
        visual_tokens = hidden_states[:, :n_visual, :]

        # Step 1: Prune — zero out tokens with lowest L2 norm
        ...
        if n_prune > 0 and n_prune < n_visual:
            # Find indices of tokens with lowest norms (to prune)
            _, prune_idx = torch.topk(norms, n_prune, dim=1, largest=False)
            # Zero out pruned tokens
            for b in range(batch):
                hidden_states[b, prune_idx[b], :] = 0.0
            self.stats["tokens_pruned"] += n_prune

            # Build mask of surviving visual token indices for merge step
            keep_mask = torch.ones(batch, n_visual, dtype=torch.bool,
                                   device=hidden_states.device)
            for b in range(batch):
                keep_mask[b, prune_idx[b]] = False
        else:
            keep_mask = torch.ones(batch, n_visual, dtype=torch.bool,
                                   device=hidden_states.device)

        # Step 2: Merge — average most-similar surviving token pairs, zero out duplicate
        # Get absolute indices of surviving visual tokens (use batch 0 as representative)
        alive_idx = keep_mask[0].nonzero(as_tuple=True)[0]  # (n_alive,)
        n_alive = alive_idx.shape[0]
        n_merge = int(n_alive * self.merge_ratio)

        if n_merge > 0 and n_alive > 2:
            alive_tokens = hidden_states[:, alive_idx, :]  # (batch, n_alive, dim)
            v_norm = nn.functional.normalize(alive_tokens.float(), dim=-1)
            sim = torch.bmm(v_norm, v_norm.transpose(1, 2))
            sim.diagonal(dim1=1, dim2=2).fill_(-1.0)

            # Track which alive slots are still active
            active = torch.ones(n_alive, dtype=torch.bool, device=hidden_states.device)

            for _ in range(min(n_merge, n_alive - 1)):
                ...
                flat_idx = sim[0].reshape(-1).argmax().item()
                i_local = flat_idx // n_alive
                j_local = flat_idx % n_alive

                if i_local == j_local or not active[i_local] or not active[j_local]:
                    break
                if sim[0, i_local, j_local] <= -1.0:
                    break

                # Merge: average into i's position, zero out j's position
                i_abs = alive_idx[i_local].item()
                j_abs = alive_idx[j_local].item()
                merged = (hidden_states[:, i_abs, :] + hidden_states[:, j_abs, :]) / 2.0
                hidden_states[:, i_abs, :] = merged
                hidden_states[:, j_abs, :] = 0.0

                # Mark j as inactive
                active[j_local] = False
                sim[:, j_local, :] = -1.0
                sim[:, :, j_local] = -1.0

                # Update i's similarities with new merged value
                merged_norm = nn.functional.normalize(merged.float(), dim=-1)  # (batch, dim)
                for k in range(n_alive):
                    if k != i_local and active[k]:
                        k_abs = alive_idx[k].item()
                        k_norm = nn.functional.normalize(
                            hidden_states[:, k_abs, :].float(), dim=-1)
                        s = (merged_norm * k_norm).sum(dim=-1)  # (batch,)
                        sim[:, i_local, k] = s
                        sim[:, k, i_local] = s
                sim[:, i_local, i_local] = -1.0
